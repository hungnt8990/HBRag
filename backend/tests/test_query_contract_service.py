...
def test_query_contract_detects_identifier_lookup() -> None:
    ...
    assert contract.preferred_artifact_types[:3] == ["document_identity", "identifier_lookup", "document_profile"]
    assert contract.output_shape == "short_fact"
    assert contract.exact_terms == ["3113"]

def test_query_contract_detects_unicode_document_code_identifier() -> None:
    contract = QueryContractService().build_contract("3113/EVN-KDMBĐ ngay nao?")

    assert contract.detected_intent == "identifier_lookup"
    assert contract.exact_terms == ["3113/EVN-KDMBĐ"]
...
def test_query_contract_detects_person_assignment() -> None:
    ...
    assert "assignment_table_row" in contract.preferred_artifact_types
...
def test_query_contract_detects_policy_rule_lookup() -> None:
    ...
    assert contract.preferred_artifact_types[0] == "legal_clause"
    assert "policy_rule_artifact" in contract.preferred_artifact_types
...
def test_query_contract_detects_procedure_lookup() -> None:
    ...
    assert "directive_task" in contract.preferred_artifact_types
    assert "procedure_artifact" in contract.preferred_artifact_types

def test_query_contract_detects_deadline_directive_lookup() -> None:
    contract = QueryContractService().build_contract("Phong Kinh te phai bao cao truoc ngay nao?")

    assert contract.detected_intent == "procedure_lookup"
    assert "deadline_requirement" in contract.preferred_artifact_types
