...
from app.services.access_control import AccessFilter, SubjectContext
...
class ArtifactFirstRetrievalService:
    ...
    def _query_phrases(query: str) -> list[str]:
        ...
        tokens = [token for token in re.findall(r"[\wÀ-ỹĐđ]+", clean, flags=re.UNICODE) if len(token) > 1]
        phrases: list[str] = []
        if clean:
