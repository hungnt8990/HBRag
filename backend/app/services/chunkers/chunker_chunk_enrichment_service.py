...
def should_llm_enrich(
    ...
    if not content.strip():
        ...
        return False, reason
    if _looks_like_table_placeholder(content):
        reason["skip"] = "table_placeholder"
        return False, reason
...
class ChunkEnrichmentService:
    ...
    async def _enrich_chunk(
        ...
        *,
        ...
        try:
            ...
            normalized = self._postprocess_payload(
                normalized,
                source_text=str(getattr(plan.chunk, "content", "") or ""),
            )
            metadata = {
                "version": self._version,
                "provider": self._provider,
    ...
    @classmethod
    def _postprocess_payload(cls, payload: dict[str, Any], *, source_text: str) -> dict[str, Any]:
        updated = dict(payload)
        updated["entities"] = [
            entity
            for entity in cls._dedupe_strings(cls._string_list(updated.get("entities")))
            if cls._appears_in_source(entity, source_text)
        ]
        return updated
...
def _looks_like_table_placeholder(content: str) -> bool:
    return bool(re.search(r"\[\[table_\d+]]", content or "", flags=re.IGNORECASE))
