...
from app.services.chunkers.chunker_heading_rule_engine import (
...
_PROFILE_ALIAS_DETECT = {
    "doffice_admin",
    "official_document_structured",
}
...
def resolve_profile_with_evidence(
    ...
    *,
    ...
    if normalized in _PROFILE_ALIAS_DETECT:
        # DOffice/official-document labels are routing aliases, not true
        # retrieval profiles. Resolve them from content so the runtime keeps the
        # same chunking/retrieval behavior as auto-detected documents.
        return detect_profile_with_evidence(
            text,
            filename=filename,
            content_type=content_type,
